...
def sequence_mask(lengths, max_len=None):
    ...
    return (
        torch.arange(0, max_len, device=lengths.device).type_as(lengths).repeat(batch_size, 1).lt(lengths.unsqueeze(1))
    )
...
def tile(x, count, dim=0):
    ...
    if isinstance(x, PackedSequence):
        ...
        perm = list(range(len(data.size())))
    else:
        data, perm = x, list(range(len(x.size())))

    if dim != 0:
        perm[0], perm[dim] = perm[dim], perm[0]
        data = data.permute(perm)

    out_size = list(data.size())
    out_size[0] *= count
    batch = data.size(0)

    data = data.contiguous().view(batch, -1).transpose(0, 1).repeat(count, 1).transpose(0, 1).contiguous().view(*out_size)
...
# Example usage:
# tiled_tensor = tile(your_input_tensor_or_packed_sequence, count=3, dim=1)
