...
class Inference(object):
    ...
    def _translate(
        ...
        # print(f'batches: {next(iter(batches))}') debug
        ...
        for batch in batches:
            # print(f"src shape: {batch.src[0].data.shape}") #debug
            # print(f"src length: {batch.src[1].shape}") #debug
            # print(f"tgt shape: {batch.tgt.data.shape}") #debug
            batch.to(corpus.device)
            batch_data = self.translate_batch(batch, None, attn_debug) #(batch, corpus.vocabs['src'], attn_debug)
    ...
    def _decode_and_generate(
        ...
        # print(f"decoder_in: {decoder_in.shape}") #debug
        if self.copy_attn:
            # Turn any copied words into UNKs.
            decoder_in = decoder_in.masked_fill(decoder_in.gt(self._tgt_vocab_len - 1), self._tgt_unk_idx)
        # print(f"decoder_in after copy_attn: {decoder_in.shape}") #debug
...
class Translator(Inference):
    ...
    def _translate_batch_with_strategy(self, batch, src_vocabs, decode_strategy):
        ...
        # print(f'gold_score: {gold_score}') #debug
        ...
        if fn_map_state is not None: #TODO: fix this
            self.model.decoder.map_state(fn_map_state)

        # (3) Begin decoding step by step:
        for step in range(decode_strategy.max_length):
            decoder_input = decode_strategy.current_predictions.view(1, -1, 1)
            # print(f"step: {step}, decoder_input: {decoder_input.shape}") #debug
            ...
            if parallel_paths > 1 or any_finished: #TODO: fix this
                self.model.decoder.map_state(
            lambda state, dim: state.index_select(dim, select_indices) if not isinstance(state, PackedSequence) else PackedSequence(state.data.index_select(0, select_indices), state.batch_sizes)
                )
