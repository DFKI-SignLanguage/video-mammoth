...
def read_examples_from_files(
    ...
    def _make_example_dict(packed):
        ...
        src_tensor, tgt_str = packed['sign'], packed['text']
        return {
            'src': src_tensor,  # Assuming src is a tensor
...
class ParallelCorpus(IterableDataset):
    ...
    def __init__(
        ...
        bos_id = self.vocabs['tgt'][DefaultTokens.BOS]
        eos_id = self.vocabs['tgt'][DefaultTokens.EOS]
        unk_id = self.vocabs['tgt'][DefaultTokens.UNK]
        pad_id = self.vocabs['tgt'][DefaultTokens.PAD]
        print(f"bos_id: {bos_id}, eos_id: {eos_id}, unk_id: {unk_id}, pad_id: {pad_id}") #debug
    ...
    def _tokenize(self, string, side='src'):
        ...
        return self.tokenizer.encode(string, out_type=str)
    ...
    def __iter__(self):
        ...
        def _cast(example_dict):
            return {
                'src': example_dict['src'],
                'tgt': self._numericalize(example_dict['tgt'], side='tgt')
            }
    ...
    def collate_fn(self, examples):
        ...
        frames = [ex['src'] for ex in examples]
        src_lengths = torch.tensor([frame.shape[0] for frame in frames], device='cpu')
        # Find the maximum length along dim=0
        max_len = max(frame.shape[0] for frame in frames)

        # Pad tensors to the maximum length
        padded_frames = []
        for frame in frames:
            pad_len = max_len - frame.shape[0]
            padded_frame = torch.nn.functional.pad(frame, (0, 0, 0, pad_len))
            padded_frames.append(padded_frame)

        # Convert the list of padded tensors to a single tensor
        padded_tensor = torch.stack(padded_frames, dim=0)
        src = (padded_tensor, src_lengths)                                              
...
def get_corpus(opts, task, src_vocab: Vocab, tgt_vocab: Vocab, is_train: bool = False):
    ...
    dataset = ParallelCorpus(
        ...
        is_train=is_train,
        task=task,
        tokenizer_model=corpus_opts["tokenizer_model"],
    )
    return dataset
