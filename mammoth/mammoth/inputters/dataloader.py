...
def build_dataloader(dataset, batch_size, batch_type, pool_size=None, n_buckets=None, cycle=True, as_iter=True):
    ...
    if not cycle:
        ...
        # loader = DataLoader(dataset, batch_size=2, collate_fn=dataset.collate_fn, shuffle=False)
    else:
        if batch_type == 'sents':
            n_buckets = 1
            ...
            def bucket_fn(example_dict):
                ...
                src_len = min(len(example_dict['src']), n_buckets)
...
class LookAheadBucketing():
    ...
    def maybe_replenish(self):
        ...
        try:
            ...
            if example is isinstance(example, tuple):
    ...
    def __iter__(self):
        while True:
            ...
            while not (batch_is_complete or self.is_empty()):
                ...
                # print(f"example. src shape: {example['src'].shape} tgt shape: {example['tgt'].shape} numel: {numel} cur_batch_size: {cur_batch_size} batch_size: {self.batch_size}") # DEBUG
    ...
    def _init_datasets(self):
        ...
        for task in self.task_queue_manager.get_tasks():
            ...
            if self.is_train or self.opts.tasks[task.corpus_id].get('path_valid_src', None) is not None:
                ...
                # print(f'corpus_id: {task.corpus_id} ordered_iter: {next(iter(ordered_iter))} metadata: {metadata}') # DEBUG
        ...
        if not self.is_train:
            ...
            while True:
                for corpus_id in self.task_queue_manager.sample_corpus_ids(communication_batch_id):
                    ...
                    # print(f'corpus_id: {corpus_id} batch: {batch} metadata: {metadata}') # DEBUG
                    if communication_batch_id == 0:
                        # De-numericalize a few sentences for debugging
                        logger.warning(
                            f'src shape: {batch.src[0].data.shape} tgt shape: {batch.tgt.shape} '
